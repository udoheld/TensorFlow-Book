...
class QLearningDecisionPolicy(DecisionPolicy):
    def __init__(self, actions, input_dim):
        self.epsilon = 0.9 #Set the hyper-parameters from the Q-function
        self.gamma = 0.001 #Set the hyper-parameters from the Q-function
        self.actions = actions
        output_dim = len(actions)
        h1_dim = 200 #Set the number of hidden nodes in the neural networks

        self.x = tf.placeholder(tf.float32, [None, input_dim]) #Define the input and output tensors
        self.y = tf.placeholder(tf.float32, [output_dim]) #Define the input and output tensors
        W1 = tf.Variable(tf.random_normal([input_dim, h1_dim])) #Design the neural network architecture
        b1 = tf.Variable(tf.constant(0.1, shape=[h1_dim])) #Design the neural network architecture
        h1 = tf.nn.relu(tf.matmul(self.x, W1) + b1) #Design the neural network architecture
        W2 = tf.Variable(tf.random_normal([h1_dim, output_dim])) #Design the neural network architecture
        b2 = tf.Variable(tf.constant(0.1, shape=[output_dim])) #Design the neural network architecture
        self.q = tf.nn.relu(tf.matmul(h1, W2) + b2) #Define the op to compute the utility

        loss = tf.square(self.y - self.q) #Set the loss as the square error
        self.train_op = tf.train.AdagradOptimizer(0.01).minimize(loss) #Use an optimizer to update model parameters to minimize the loss
        self.sess = tf.Session() #Set up the session and initialize variables
        self.sess.run(tf.initialize_all_variables()) #Set up the session and initialize variables
    ...
    def update_q(self, state, action, reward, next_state): #Update the Q-function by updating its model parameters
